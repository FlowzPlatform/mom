          ...
          <span class="dreg-move"></span>
                <input id="checkbox8" type="checkbox" checked="" v-model="todo.completed" class="toggle" @change="checkboxToggle">
                  <label for="checkbox8">
						...
						<input 
						class="new-todo" autofocus autocomplete="off" :placeholder="pholder" 
						v-model="todo.taskName"
            @click="showDiv(todo.level, parentIdArr)"
            @focus="getIndex(eventIndexR)"
            @keyup.enter="handleAddTodo">
            <span class=""><i>
              <b class="glyphicon glyphicon-option-vertical"></b><b class="glyphicon glyphicon-option-vertical"></b>
              </i></span>
...
export default {
  ...
  methods: {
    ...
    showDiv: function(level, parentIdArr){
        ...
        if(parentTaskId)
        {
            var parentIdArrObj = {"id": parentTaskId, "level":level, "parentTaskName": parentTaskName, "parentTaskDesc" : parentTaskDesc, "parentTaskComment" : parentTaskComment}
