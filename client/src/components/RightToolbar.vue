              ...
              <span class="token user_token">
                ...
                <span class="dropdown">
                  ...
                  <ul class='dropdown-menu userlist' aria-labelledby="userlist">
                    <li v-for="(user, index) in getUserList">
                      <a @click="btnUserClicked(user)">
                        <span>
                          <img v-if="user.image_url" v-bind:src="user.image_url" />
                          <div v-else>{{user.email | capitalizeLetters}}</div>
                        </span>{{user.email}}</a>
                      <hr>
                    </li>
                  </ul>
        ...
        <span class="dropdown">
          ...
          <ul class="dropdown-menu typeList">
            <li v-for='type in getTypes'>
              <a @click="btnTypeClicked(type)">{{type.type}}</a>
              <hr>
            </li>
          </ul>
        </span>
        ...
        <div class="redesign-due-date-container">
          <div class="property due_date value-set">
            <div class="property-name">
              <span>
                <datepicker placeholder="Due Date" class="wrapperClass temp" v-on:selected="dateFormatter" clear-button calendarButton calendarButtonIcon="fa fa-calendar"
                  clearButtonIcon="fa fa-close" today selected v-model="filteredTodo.dueDate">
                </datepicker>
              </span>
            </div>
          </div>
      ...
      <div class="loading-boundary reskinToolbarViewsWithShortcuts">
          ...
          <span class="heart-button heartButtonView heartButtonView--withHearts">
            <div class="heartButtonView-container ">
              <div class="circularToggleButtonView  circularToggleButtonView--toggledOff heartToggleButtonView">
                ...
                <div class="circularButtonView circularToggleButtonView-button circularButtonView--default circularButtonView--onWhiteBackground circularButtonView--active"
                  id="luna16">
        ...
        <span class="attachmentsMenuView dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="details_property_sheetproperty_attach_attach_menu">
            <li>
              <a id="add_from_computer" for="attachments_menu_view_hidden_file_input_3" class="menu-item" title="">
                <span class="dropdown-menu-item-label upload-file-dropdown">
                  <input type="file" id="file" @change="onFileChange($event)" @click="removeAttachmentPopUp">
